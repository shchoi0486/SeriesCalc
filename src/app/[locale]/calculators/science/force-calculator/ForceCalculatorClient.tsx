...
const infoSection = (isKo: boolean) => {
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="my-2 p-3 bg-muted rounded-lg">
          <BlockMath math="F = 1{,}000\,\text{kg} \times 3\,\text{m/s}^2 = 3{,}000\,\text{N}" />
        </div>
  ...
  howToUse: (
    <ol className="space-y-3 text-sm list-decimal list-inside">
      ...
      <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">{L('가속도 입력', 'Enter acceleration')}</span>{L(' → 물체의 가속도를 m/s² 단위로 입력합니다.', ' → Enter the acceleration in meters per second squared (m/s²).')}</li>
      ...
      <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">{L('결과 확인', 'View result')}</span>{L(' → 필요한 힘의 크기를 뉴턴(N) 단위로 확인합니다.', ' → Check the required force in newtons (N).')}</li>
    </ol>
  ),
  workedExamples: (
    <div className="space-y-3 text-sm">
      <div className="p-3 bg-muted rounded-lg">
        <p className="font-semibold">{L('10kg 물체의 무게', 'Weight of a 10 kg object')}</p>
        <p className="text-muted-foreground mt-1">F = 10kg × 9.8m/s² = 98N</p>
      </div>
      <div className="p-3 bg-muted rounded-lg">
        <p className="font-semibold">{L('자동차 가속', 'Car acceleration')}</p>
        <p className="text-muted-foreground mt-1">F = 1500kg × 2m/s² = 3000N</p>
      </div>
      <div className="p-3 bg-muted rounded-lg">
        <p className="font-semibold">{L('100g 물체의 가속', '100 g object accelerating')}</p>
        <p className="text-muted-foreground mt-1">100g × 5m/s² = 0.5N</p>
      </div>
    </div>
  ),
  faq: (
    <div className="space-y-4">
      {[
        { q: L('F=ma 공식은 무엇을 의미하나요?', 'What does F = ma mean?'), a: L('뉴턴의 제2운동법칙인 F=ma는 물체에 가해지는 알짜힘(F)이 물체의 질량(m)과 가속도(a)의 곱과 같다는 것을 나타냅니다. 힘이 클수록, 또는 질량이 작을수록 가속도가 커집니다.', 'F = ma is Newton’s second law of motion, stating that the net force (F) acting on an object equals the product of its mass (m) and acceleration (a). The greater the force, or the smaller the mass, the greater the acceleration.') },
        { q: L('왜 kg × m/s²이 뉴턴(N)이 되나요?', 'Why does kg × m/s² become newtons (N)?'), a: L('1N은 1kg의 질량을 1m/s²의 가속도로 가속시키는 데 필요한 힘으로 정의되므로, kg × m/s²은 자연스럽게 힘의 국제 표준 단위인 뉴턴(N)이 됩니다.', 'One newton is defined as the force needed to accelerate a 1 kg mass at 1 m/s², so kg × m/s² naturally becomes the international standard unit of force, the newton (N).') },
        { q: L('질량과 무게의 차이는 무엇인가요?', 'What is the difference between mass and weight?'), a: L('질량은 물체 고유의 물질량으로 중력과 무관하게 일정합니다. 무게는 질량에 중력가속도(g)를 곱한 힘(W = mg)으로, 중력이 다른 곳에서는 달라질 수 있습니다.', 'Mass is an object’s intrinsic amount of matter and stays constant regardless of gravity. Weight is the force W = mg, so it depends on gravity and can differ from place to place.') },
        { q: L('알짜힘과 개별 힘의 차이는 무엇인가요?', 'What is the difference between net force and individual forces?'), a: L('알짜힘은 물체에 작용하는 모든 개별 힘을 벡터로 합한 결과입니다. 여러 힘이 서로 상쇄될 수 있으므로, 물체의 실제 가속도는 개별 힘이 아닌 알짜힘에 의해 결정됩니다.', 'Net force is the vector sum of all individual forces acting on an object. Since forces can cancel each other out, the object’s actual acceleration is determined by the net force, not by individual forces.') },
        { q: L('뉴턴의 제2운동법칙은 어디에 활용되나요?', 'Where is Newton’s second law applied?'), a: L('자동차 설계, 로켓 추진, 충돌 안전 분석, 스포츠 과학, 구조물 하중 해석 등 공학과 과학 전반에서 힘과 운동을 예측하는 데 널리 활용됩니다.', 'Newton’s second law is widely used across engineering and science — from car design, rocket propulsion, and crash safety analysis to sports science and structural load analysis — to predict force and motion.') },
