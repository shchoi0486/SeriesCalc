    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="p-4 bg-muted rounded-lg">
          <BlockMath math="\tau = r F \sin(\theta)" />
        </div>
      ...
      <div>
        ...
        <div className="my-2 p-3 bg-muted rounded-lg">
          <BlockMath math="\tau = 0.5\,\text{m} \times 50\,\text{N} = 25\,\text{N·m}" />
        </div>
  ...
  howToUse: (
    <div className="space-y-4">
      <ol className="list-decimal list-inside space-y-2">
        <li>{L('가해지는 힘을 뉴턴(N) 단위로 입력하세요.', 'Enter the applied force in newtons (N).')}</li>
        <li>{L('회전축(지점)에서 힘이 작용하는 지점까지의 거리를 미터(m) 단위로 입력하세요.', 'Enter the distance from the pivot point to where the force is applied, in meters (m).')}</li>
        <li>{L('힘과 레버 암 사이의 각도를 도(deg) 단위로 입력하세요.', 'Enter the angle between the force and the lever arm in degrees.')}</li>
        <li>{L('계산 버튼을 클릭하면 토크가 뉴턴미터(N·m) 단위로 계산됩니다.', 'Click calculate and the torque is computed in newton-meters (N·m).')}</li>
      </ol>
    </div>
  ),
  workedExamples: (
    <div className="space-y-4">
      <div className="p-4 bg-muted rounded-lg border-l-4 border-primary">
        <h4 className="font-bold text-lg mb-2">{L('10N × 2m @ 90°', '10 N × 2 m at 90°')}</h4>
        <p>{L('회전축에서 2m 떨어진 곳에 90° 각도로 10N의 힘을 가하면 토크는 10 × 2 × sin(90°) = 20 N·m입니다.', 'Applying a 10 N force at 90° at a distance of 2 m from the axis gives a torque of 10 × 2 × sin(90°) = 20 N·m.')}</p>
      </div>
      <div className="p-4 bg-muted rounded-lg border-l-4 border-primary">
        <h4 className="font-bold text-lg mb-2">{L('50N × 0.3m @ 90°', '50 N × 0.3 m at 90°')}</h4>
        <p>{L('0.3m 렌치에 90° 각도로 50N의 힘을 가하면 토크는 50 × 0.3 × sin(90°) = 15 N·m입니다.', 'Applying a 50 N force at 90° to a 0.3 m wrench gives a torque of 50 × 0.3 × sin(90°) = 15 N·m.')}</p>
      </div>
      <div className="p-4 bg-muted rounded-lg border-l-4 border-primary">
        <h4 className="font-bold text-lg mb-2">{L('20N × 1m @ 30°', '20 N × 1 m at 30°')}</h4>
        <p>{L('1m 거리에서 30° 각도로 20N의 힘을 가하면 sin(30°) = 0.5이므로 토크는 20 × 1 × 0.5 = 10 N·m입니다.', 'Applying a 20 N force at 30° at a distance of 1 m, since sin(30°) = 0.5, gives a torque of 20 × 1 × 0.5 = 10 N·m.')}</p>
      </div>
    </div>
  ),
  faq: (
    <div className="space-y-4">
      {[
        {
          q: L('토크란 무엇인가요?', 'What is torque?'),
          a: L('토크는 회전축을 중심으로 물체를 돌리려는 힘의 경향을 나타내는 물리량으로, 회전 운동에서 힘에 해당하는 개념입니다. 단위는 뉴턴미터(N·m)입니다.', 'Torque is a physical quantity that describes the tendency of a force to rotate an object about an axis — the rotational analogue of force. Its unit is the newton-meter (N·m).'),
        },
        {
          q: L('토크 공식은 무엇인가요?', 'What is the torque formula?'),
          a: L('토크는 τ = rF sin(θ)로 계산합니다. 여기서 r은 회전축까지의 거리(레버 암), F는 가해지는 힘, θ는 힘과 레버 암 사이의 각도입니다.', 'Torque is calculated as τ = rF sin(θ), where r is the distance from the axis (lever arm), F is the applied force, and θ is the angle between the force and the lever arm.'),
        },
        {
          q: L('N·m 단위는 무엇을 의미하나요?', 'What does the unit N·m mean?'),
          a: L('N·m(뉴턴미터)는 1뉴턴의 힘을 1미터의 팔길이에 수직으로 작용시켰을 때의 토크입니다. 토크와 에너지가 같은 단위처럼 보이지만, 토크는 벡터량이고 에너지는 스칼라량입니다.', 'N·m (newton-meter) is the torque from a 1 newton force applied perpendicularly at a 1 meter arm. Although torque and energy share the same-looking unit, torque is a vector while energy is a scalar.'),
        },
        {
          q: L('각도가 왜 중요하나요?', 'Why does the angle matter?'),
          a: L('토크는 힘의 수직 성분에 비례하므로, 힘이 레버 암에 수직(90°)일 때 토크가 최대가 됩니다. 각도가 0°나 180°이면 토크는 0이 되고, 비스듬할수록 실제 회전 효과는 줄어듭니다.', 'Torque is proportional to the perpendicular component of the force, so it is maximum when the force is perpendicular (90°) to the lever arm. At 0° or 180° the torque is zero, and oblique angles reduce the effective rotation.'),
        },
        {
          q: L('토크와 힘의 차이는 무엇인가요?', 'What is the difference between torque and force?'),
          a: L('힘은 물체를 직선으로 밀거나 당기는 작용이고, 토크는 물체를 회전시키는 작용입니다. 같은 크기의 힘이라도 회전축으로부터의 거리와 각도에 따라 토크가 달라집니다.', 'Force pushes or pulls an object in a straight line, while torque rotates an object about an axis. The same force produces different torque depending on the distance from the axis and the angle.'),
        },
      ].map((f, i) => (
