...
const CalorieCalculator = () => {
  ...
  const calculate = () => {
    ...
    if (isNaN(a) || isNaN(h) || isNaN(w) || a <= 0 || h <= 0 || w <= 0) {
      toast.error(isKo ? '모든 값을 올바르게 입력해주세요.' : 'Please enter all values correctly.');
      return;
    }
        ...
        <div>
          ...
          <ol className="space-y-4 text-sm text-muted-foreground">
            <li className="flex gap-3">
              ...
              <div><strong>신체 정보 입력</strong> &rarr; 성별, 키(cm), 체중(kg), 나이(세)를 입력합니다</div>
            </li>
            <li className="flex gap-3">
              ...
              <div><strong>활동량 선택</strong> &rarr; 일상 활동 수준을 선택합니다</div>
            </li>
            <li className="flex gap-3">
              ...
              <div><strong>목표 설정</strong> &rarr; 감량/유지/증량 중 목표와 주당 목표 체중 변화를 설정합니다</div>
            </li>
            <li className="flex gap-3">
              ...
              <div><strong>결과 확인</strong> &rarr; 기초대사율, 일일 소비 칼로리, 목표 칼로리, 예상 기간을 확인합니다</div>
            </li>
          </ol>
        </div>
      ),
      workedExamples: (
        <div>
          ...
          <div className="space-y-6 text-sm text-muted-foreground">
            <div className="p-3 bg-card rounded-md shadow-sm border border-border">
              <p className="font-semibold text-primary">예시 1: 30세 남성, 175cm, 70kg, 보통 활동, 감량 0.5kg/주</p>
              <code className="block bg-muted p-2 rounded-md my-2 text-xs">
                BMR = 1683 kcal, TDEE = 2609 kcal
              </code>
              <code className="block bg-muted p-2 rounded-md my-2 text-xs">
                목표 칼로리 = 2609 - 500 = 2109 kcal/일
              </code>
              <p className="text-xs">예상 기간: 약 <strong>14주</strong></p>
            </div>
            <div className="p-3 bg-card rounded-md shadow-sm border border-border">
              <p className="font-semibold text-primary">예시 2: 25세 여성, 160cm, 55kg, 가벼운 활동, 감량 0.5kg/주</p>
              <code className="block bg-muted p-2 rounded-md my-2 text-xs">
                BMR = 1304 kcal, TDEE = 1793 kcal
              </code>
              <code className="block bg-muted p-2 rounded-md my-2 text-xs">
                목표 칼로리 = 1793 - 500 = 1293 kcal/일
              </code>
              <p className="text-xs">예상 기간: 약 <strong>11주</strong></p>
            </div>
          </div>
        </div>
      ),
      faq: (
        <div>
          ...
          <div className="space-y-5 text-sm text-muted-foreground">
            <FaqItem q="하루 최소 칼로리는 얼마인가요?" a="남성은 약 1500kcal, 여성은 약 1200kcal가 최소 기준입니다. 이 이하로 섭취하면 영양 결핍과 대사율 저하가 발생할 수 있습니다." />
            <FaqItem q="체중 감량 속도는 얼마나 빠르게 해야 하나요?" a="안전한 감량 속도는 주 0.5~1kg입니다. 너무 빠르면 근육 손실, 담석증, 체중 재증가 위험이 있습니다." />
            <FaqItem q="탄수화물/단백질/지방 비율은 어떻게 되나요?" a="일반적으로 탄수화물 45~65%, 단백질 10~35%, 지방 20~35%가 적절합니다. 감량 시 높은 단백질(1.6~2.2g/kg)이 근육량 유지에 도움이 됩니다." />
            <FaqItem q="운동 없이 다이어트가 가능한가요?" a="가능하지만 더 어렵습니다. 식단만으로 적자를 만들 수 있지만, 운동과 병행하면 근육량을 유지하고 체 구성 개선에 도움이 됩니다." />
            <FaqItem q="Plateau(정체기)는 왜 발생하나요?" a="대사 적응으로 체중이 줄어들면서 신체가 에너지 소비를 줄입니다. 해결책: TDEE를 재계산하고, 섭취를 조정하며, 운동을 다양하게 변경하세요." />
          </div>
        </div>
      ),
    }
  ) : (
    {
      ...
      howToUse: (
        <div>
          ...
          <ol className="space-y-4 text-sm text-muted-foreground">
            <li className="flex gap-3">
              <span className="shrink-0 w-6 h-6 rounded-full bg-primary/10 text-primary text-xs font-bold flex items-center justify-center">1</span>
              <div><strong>Enter body info</strong> &rarr; input sex, height (cm), weight (kg), and age (years)</div>
            </li>
            <li className="flex gap-3">
              <span className="shrink-0 w-6 h-6 rounded-full bg-primary/10 text-primary text-xs font-bold flex items-center justify-center">2</span>
              <div><strong>Select activity level</strong> &rarr; choose your daily activity level</div>
            </li>
            <li className="flex gap-3">
              <span className="shrink-0 w-6 h-6 rounded-full bg-primary/10 text-primary text-xs font-bold flex items-center justify-center">3</span>
              <div><strong>Set goal</strong> &rarr; choose loss/maintain/gain and target weekly weight change</div>
            </li>
            <li className="flex gap-3">
              <span className="shrink-0 w-6 h-6 rounded-full bg-primary/10 text-primary text-xs font-bold flex items-center justify-center">4</span>
              <div><strong>View results</strong> &rarr; check BMR, daily calorie expenditure, target calories, and estimated duration</div>
            </li>
          </ol>
        </div>
      ),
      workedExamples: (
        <div>
          ...
          <div className="space-y-6 text-sm text-muted-foreground">
            <div className="p-3 bg-card rounded-md shadow-sm border border-border">
              <p className="font-semibold text-primary">Example 1: 30-year-old male, 175cm, 70kg, moderate activity, lose 0.5kg/week</p>
              <code className="block bg-muted p-2 rounded-md my-2 text-xs">
                BMR = 1683 kcal, TDEE = 2609 kcal
              </code>
              <code className="block bg-muted p-2 rounded-md my-2 text-xs">
                Target = 2609 - 500 = 2109 kcal/day
              </code>
              <p className="text-xs">Estimated duration: approximately <strong>14 weeks</strong></p>
            </div>
            <div className="p-3 bg-card rounded-md shadow-sm border border-border">
              <p className="font-semibold text-primary">Example 2: 25-year-old female, 160cm, 55kg, light activity, lose 0.5kg/week</p>
              <code className="block bg-muted p-2 rounded-md my-2 text-xs">
                BMR = 1304 kcal, TDEE = 1793 kcal
              </code>
              <code className="block bg-muted p-2 rounded-md my-2 text-xs">
                Target = 1793 - 500 = 1293 kcal/day
              </code>
              <p className="text-xs">Estimated duration: approximately <strong>11 weeks</strong></p>
            </div>
          </div>
        </div>
      ),
      faq: (
        <div>
          ...
          <div className="space-y-5 text-sm text-muted-foreground">
            <FaqItem q="What is the minimum daily calorie intake?" a="Men: ~1500 kcal, Women: ~1200 kcal minimum. Below this, nutritional deficiencies and metabolic slowdown occur." />
            <FaqItem q="How fast should I lose weight?" a="Safe rate: 0.5-1 kg per week. Faster loss risks muscle loss, gallstones, and weight regain." />
            <FaqItem q="What should my carb/protein/fat ratio be?" a="General: 45-65% carbs, 10-35% protein, 20-35% fat. Higher protein (1.6-2.2g/kg) helps preserve muscle during deficit." />
            <FaqItem q="Can I diet without exercise?" a="Possible but harder. Diet alone creates deficit, but combining with exercise preserves muscle mass and improves body composition." />
            <FaqItem q="Why do plateaus occur?" a="Metabolic adaptation: body reduces energy expenditure as weight drops. Solution: recalculate TDEE, adjust intake, vary exercise." />
          </div>
        </div>
      ),
    }
