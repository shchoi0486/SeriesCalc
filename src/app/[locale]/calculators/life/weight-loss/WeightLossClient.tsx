  ...
  const infoSection = isKo ? (
    {
      ...
      howToUse: (
        <ol className="space-y-3 text-sm list-decimal list-inside">
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">신체 정보 입력</span> → 성별, 키(cm), 체중(kg), 나이(세)를 입력합니다</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">활동량 선택</span> → 일상 활동 수준을 선택합니다</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">목표 설정</span> → 감량 목표와 주당 목표 변화량을 설정합니다</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">결과 확인</span> → 기초대사율, 일일 소비 칼로리, 목표 칼로리, 예상 기간을 확인합니다</li>
        </ol>
      ),
      workedExamples: (
        <div className="space-y-3 text-sm">
          <div className="p-3 bg-muted rounded-lg">
            <p className="font-semibold">30세 남성 175cm 70kg 보통 활동, 감량 0.5kg/주</p>
            <p className="text-muted-foreground mt-1">BMR=1683, TDEE=2609, 목표=2109kcal/일, 예상 14주</p>
          </div>
          <div className="p-3 bg-muted rounded-lg">
            <p className="font-semibold">25세 여성 160cm 55kg 가벼운 활동, 감량 0.5kg/주</p>
            <p className="text-muted-foreground mt-1">BMR=1304, TDEE=1793, 목표=1293kcal/일, 예상 11주</p>
          </div>
        </div>
      ),
      faq: (
        <div className="space-y-4">
          {[
            { q: '하루 최소 칼로리는 얼마인가요?', a: '남성 약 1500kcal, 여성 약 1200kcal이 최소 기준입니다. 이 이하로는 대사 저하와 영양 결핍 위험이 있습니다.' },
            { q: '체중 감량 속도는 얼마나 빠르게?', a: '안전한 속도는 주 0.5~1kg입니다. 너무 빠른 감량은 근육량 손실, 담낭 결석, 요요 현상을 유발합니다.' },
            { q: '탄수화물/단백질/지방 비율은?', a: '탄수화물 45-65%, 단백질 10-35%, 지방 20-35%. 적자 상태에서는 단백질을 충분히(1.6-2.2g/kg) 섭취해야 근육을 보존할 수 있습니다.' },
            { q: '운동 없이 다이어트 가능한가요?', a: '가능하지만 식단만으로는 근육량 손실 위험이 있습니다. 운동을 병행하면 근육을 보존하고 체형을 개선할 수 있습니다.' },
            { q: '정체기(Plateau)는 왜 발생하나요?', a: '대사 적응: 체중이 감소하면 몸이 에너지 소비를 줄입니다. TDEE를 다시 계산하고 섭취량을 조정해야 합니다.' },
  ...
  ) : (
    {
      ...
      howToUse: (
        <ol className="space-y-3 text-sm list-decimal list-inside">
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">Enter body info</span> → Enter sex, height (cm), weight (kg), age (years)</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">Select activity level</span> → Choose your daily activity level</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">Set goals</span> → Set your weight loss target and weekly goal rate</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">View results</span> → Check BMR, daily calorie expenditure, target calories, and estimated duration</li>
        </ol>
      ),
      workedExamples: (
        <div className="space-y-3 text-sm">
          <div className="p-3 bg-muted rounded-lg">
            <p className="font-semibold">Male, 30y, 175cm, 70kg, Moderate activity, 0.5kg/week loss</p>
            <p className="text-muted-foreground mt-1">BMR=1683, TDEE=2609, Target=2109kcal/day, ~14 weeks</p>
          </div>
          <div className="p-3 bg-muted rounded-lg">
            <p className="font-semibold">Female, 25y, 160cm, 55kg, Light activity, 0.5kg/week loss</p>
            <p className="text-muted-foreground mt-1">BMR=1304, TDEE=1793, Target=1293kcal/day, ~11 weeks</p>
          </div>
        </div>
      ),
      faq: (
        <div className="space-y-4">
          {[
            { q: 'What is the minimum daily calorie intake?', a: 'Men ~1500 kcal, Women ~1200 kcal minimum. Below this risks metabolic slowdown and nutritional deficiency.' },
            { q: 'How fast should I lose weight?', a: 'Safe: 0.5-1 kg/week. Faster loss causes muscle loss, gallstones, and weight regain.' },
            { q: 'What are the ideal carb/protein/fat ratios?', a: '45-65% carbs, 10-35% protein, 20-35% fat. Higher protein (1.6-2.2g/kg) preserves muscle during deficit.' },
            { q: 'Can I diet without exercise?', a: 'Possible but diet alone risks muscle loss. Combined exercise preserves muscle and improves body composition.' },
            { q: 'Why do plateaus occur?', a: 'Metabolic adaptation: body reduces expenditure as weight drops. Recalculate TDEE and adjust intake.' },
          ].map((f, i) => <FaqItem key={i} q={f.q} a={f.a} />)}
        </div>
      ),
    }
