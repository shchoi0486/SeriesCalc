...
const KoreanAgeCalculator = () => {
  ...
  const calculateKoreanAge = () => {
    if (calculationMethod === 'birthDate') {
      if (!birthDate) {
        toast.error(t.placeholders.birthday);
        return;
      }
  ...
  const infoSection = isKo ? (
    ...
    ,
      howToUse: (
        <ol className="space-y-3 text-sm list-decimal list-inside">
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">생년월일 선택</span> → 생년월일 또는 출생 연도를 입력합니다</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">오늘 날짜 자동 적용</span> → 기준일은 오늘 날짜로 자동 설정됩니다</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">계산 실행</span> → 계산 버튼을 클릭하면 세 가지 나이를 확인할 수 있습니다</li>
        </ol>
      ),
      workedExamples: (
        <div className="space-y-3 text-sm">
          <div className="p-3 bg-muted rounded-lg">
            <p className="font-semibold">생일: 2000-01-15, 오늘: 2026-08-25</p>
            <p className="text-muted-foreground mt-1">한국 나이 27세, 만 나이 26세, 연 나이 26세</p>
          </div>
        </div>
      ),
      faq: (
        <div className="space-y-4">
          {[
            { q: '한국 나이, 만 나이, 연 나이의 차이는?', a: '한국 나이는 태어나면 1세, 매년 1월 1일마다 1세 추가. 만 나이는 생일 기준 0세부터 시작. 연 나이는 현재 연도-출생 연도로 생일 무관.' },
            { q: '2023년 만 나이 통일법은 무엇인가요?', a: '2023년 6월 28일부터 시행. 법률, 계약, 공문서에서 만 나이만 사용하도록 명시. 다만 병역법, 청소년보호법 등 일부는 연 나이 유지.' },
            { q: '국제적으로 사용되는 나이는?', a: '만 나이(국제 나이)가 전 세계 표준입니다. 의료, 여권, 비자 등에서는 만 나이를 사용하며, 한국도 법적으로 통일되었습니다.' },
            { q: '생일 계산은 어떻게 하나요?', a: '한국 나이: 올해 연도-출생 연도+1. 만 나이: 올해 연도-출생 연도 (생일 전이면 -1). 연 나이: 올해 연도-출생 연도.' },
            { q: '술, 담배, 투표 가능 나이는?', a: '청소년 보호법상 연 나이 19세 이상부터 술, 담배 구매 가능. 투표는 만 18세 이상. 출생 연도 기준 1월 1일부터 적용.' },
    ...
    `,
      howToUse: (
        <ol className="space-y-3 text-sm list-decimal list-inside">
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">Select birth date</span> → Enter your date of birth or birth year</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">Today&apos;s date auto-applied</span> → The reference date is automatically set to today</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">Calculate</span> → Click Calculate to see all three ages at once</li>
        </ol>
      ),
      workedExamples: (
        <div className="space-y-3 text-sm">
          <div className="p-3 bg-muted rounded-lg">
            <p className="font-semibold">Born: 2000-01-15, Today: 2026-08-25</p>
            <p className="text-muted-foreground mt-1">Korean age 27, international age 26, year age 26</p>
          </div>
        </div>
      ),
      faq: (
        <div className="space-y-4">
          {[
            { q: 'What is the difference between Korean age, international age, and year age?', a: 'Korean age: 1 at birth, +1 every January 1. International age: 0 at birth, +1 each birthday. Year age: current year minus birth year, regardless of birthday.' },
            { q: 'What is the 2023 Korean age law change?', a: 'Effective June 28, 2023, only international age is used in laws, contracts, and official documents. However, laws such as the Military Service Act and Youth Protection Act still use year age.' },
            { q: 'Which age is used internationally and medically?', a: 'International age is the global standard, used in medical care, passports, and visas. South Korea has also unified its legal standard to international age.' },
            { q: 'How is each age calculated based on the birthday?', a: 'Korean age: current year - birth year + 1. International age: current year - birth year (minus 1 if your birthday has not passed yet). Year age: current year - birth year.' },
            { q: 'What are the drinking and voting age laws?', a: 'Under the Youth Protection Act, alcohol and tobacco can be purchased from year age 19. Voting requires age 18 or older by international age. Both apply from January 1 based on the birth year.' },
          ].map((f, i) => <FaqItem key={i} q={f.q} a={f.a} />)}
        </div>
      ),
    }
