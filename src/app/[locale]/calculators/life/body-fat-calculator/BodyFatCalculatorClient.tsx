...
const BodyFatCalculator = () => {
  ...
  const calculate = () => {
    ...
    if (isNaN(a) || isNaN(h) || isNaN(w) || isNaN(n) || isNaN(wa)) {
      toast.error(isKo ? '필수 값을 모두 입력해주세요.' : 'Please enter all required values.');
      return;
    }

    if (gender === 'female' && isNaN(hipVal)) {
      toast.error(isKo ? '여성은 엉덩이둘레를 입력해주세요.' : 'Females must enter hip circumference.');
      return;
    }
    ...
    if (isNaN(bodyFat) || bodyFat < 0 || bodyFat > 60) {
      toast.error(isKo ? '입력값을 확인해주세요. 측정값이 올바르지 않습니다.' : 'Please check your input values. Measurements seem incorrect.');
      return;
    }
  ...
  const infoSection = isKo ? (
    {
      ...
      howToUse: (
        <ol className="space-y-3 text-sm list-decimal list-inside">
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">성별 선택</span> → 남성/여성을 선택합니다</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">측정값 입력</span> → 목둘레, 허리둘레를 cm 단위로 입력합니다 (여성은 엉덩이둘레도 입력)</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">계산</span> → 계산 버튼을 클릭합니다</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">결과 확인</span> → 체지방률(%)과 체지방 분류(정상~비만)를 확인합니다</li>
        </ol>
      ),
      workedExamples: (
        <div className="space-y-3 text-sm">
          <div className="p-3 bg-muted rounded-lg">
            <p className="font-semibold">남성, 목 38cm, 허리 90cm</p>
            <p className="text-muted-foreground mt-1">체지방률 ≈ 495 / (90 / (38×0.415)) - 415 ≈ 약 22% → 정상 범위</p>
          </div>
          <div className="p-3 bg-muted rounded-lg">
            <p className="font-semibold">여성, 목 34cm, 허리 75cm, 엉덩이 98cm</p>
            <p className="text-muted-foreground mt-1">체지방률 ≈ 495 / (75 / (34×0.415)) - 415 ≈ 약 30% → 경도 비만</p>
          </div>
        </div>
      ),
      faq: (
        <div className="space-y-4">
          {[
            { q: 'US Navy 방법이 다른 체지방 측정법보다 정확한가요?', a: 'DEXA(약 1.5% 오차)에 비해 US Navy는 약 3~4% 오차가 있습니다. 가정에서 사용하기에 실용적이지만, 임상 기관의 측정법보다는 정확도가 떨어집니다.' },
            { q: '체지방률 기준은 어떻게 되나요?', a: '남성: 필수 지방 2-5%, 운동선수 6-13%, 건강 14-17%, 보통 18-24%, 비만 25%+. 여성: 필수 지방 10-13%, 운동선수 14-20%, 건강 21-24%, 보통 25-31%, 비만 32%+.' },
            { q: '근육량이 많으면 측정이 부정확하나요?', a: '예. 매우 근육이 많은 경우 목둘레와 허리둘레가 높게 나와 체지방을 과대추정할 수 있습니다. 운동선수에게는 피부접촉법이나 DEXA가 더 정확합니다.' },
            { q: '나이와 성별에 따른 차이는?', a: '여성은 필수 지방이 남성보다 높습니다(10-13% vs 2-5%). 호르몬 변화와 근육량 감소로 나이가 들수록 체지방률이 증가합니다.' },
            { q: '정기적으로 측정하면 어떤 변화를 볼 수 있나요?', a: '주 1~2회 추적 측정이 단일 측정보다 변화 추이를 더 정확하게 보여줍니다. 측정 조건(아침, 공복)을 일관되게 유지하면 비교 정확도가 향상됩니다.' },
  ...
  ) : (
    {
      ...
      howToUse: (
        <ol className="space-y-3 text-sm list-decimal list-inside">
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">Select sex</span> → Choose Male or Female</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">Enter measurements</span> → Enter neck and waist circumference in cm (females also enter hip)</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">Calculate</span> → Click the Calculate button</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">View results</span> → Check body fat percentage (%) and body fat classification (Normal~Obese)</li>
        </ol>
      ),
      workedExamples: (
        <div className="space-y-3 text-sm">
          <div className="p-3 bg-muted rounded-lg">
            <p className="font-semibold">Male, Neck 38cm, Waist 90cm</p>
            <p className="text-muted-foreground mt-1">Body fat ≈ 495 / (1.0324 - 0.19077×log10(90-38) + 0.15456×log10(170)) - 450 ≈ ~22% → Normal range</p>
          </div>
          <div className="p-3 bg-muted rounded-lg">
            <p className="font-semibold">Female, Neck 34cm, Waist 75cm, Hip 98cm</p>
            <p className="text-muted-foreground mt-1">Body fat ≈ 495 / (1.29579 - 0.35004×log10(75+98-34) + 0.22100×log10(160)) - 450 ≈ ~30% → Mildly Obese</p>
          </div>
        </div>
      ),
      faq: (
        <div className="space-y-4">
          {[
            { q: 'Is the US Navy method more accurate than other body fat measurements?', a: 'Compared to DEXA (~1.5% error), US Navy has ~3-4% error. Practical for home use but less accurate than clinical methods.' },
            { q: 'What are the body fat percentage ranges?', a: 'Men: Essential 2-5%, Athletes 6-13%, Fitness 14-17%, Average 18-24%, Obese 25%+. Women: Essential 10-13%, Athletes 14-20%, Fitness 21-24%, Average 25-31%, Obese 32%+.' },
            { q: 'Is the measurement inaccurate for very muscular individuals?', a: 'Yes. Very muscular individuals may have higher neck/waist measurements, potentially overestimating body fat. Skin-fold or DEXA is more reliable for athletes.' },
            { q: 'What are the differences by age and sex?', a: 'Women naturally have higher essential fat (10-13% vs 2-5% for men). Body fat increases with age due to hormonal changes and decreased muscle mass.' },
            { q: 'What changes can regular measurements reveal?', a: 'Tracking weekly/biweekly shows trends more accurately than single measurements. Consistency in measurement conditions (morning, fasted) improves comparability.' },
          ].map((f, i) => <FaqItem key={i} q={f.q} a={f.a} />)}
        </div>
      ),
    }
