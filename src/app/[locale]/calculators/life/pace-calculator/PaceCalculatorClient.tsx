...
const PaceCalculator = () => {
  ...
  const calculate = () => {
    ...
    if (mode === 'distance-time') {
      if (isNaN(d) || d <= 0) {
        toast.error(isKo ? '거리를 올바르게 입력해주세요.' : 'Please enter a valid distance.');
        return;
      }
      const totalSec = h * 3600 + m * 60 + s;
      if (totalSec <= 0) {
        toast.error(isKo ? '시간을 올바르게 입력해주세요.' : 'Please enter a valid time.');
        return;
      }
      ...
      if (paceTotal <= 0) {
        toast.error(isKo ? '페이스를 올바르게 입력해주세요.' : 'Please enter a valid pace.');
        return;
      }
  ...
  const infoSection = isKo ? (
    {
      ...
      howToUse: (
        <ol className="space-y-3 text-sm list-decimal list-inside">
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">모드 선택</span> → 속도→페이스 또는 페이스→속도 변환 모드를 선택합니다</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">값 입력</span> → 거리(km), 시간 또는 페이스(분:초/km)를 입력합니다</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">변환 실행</span> → 계산 버튼을 클릭하여 변환을 실행합니다</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">결과 확인</span> → 계산된 페이스/속도와 참고 러닝 페이스를 확인합니다</li>
        </ol>
      ),
      workedExamples: (
        <div className="space-y-3 text-sm">
          <div className="p-3 bg-muted rounded-lg">
            <p className="font-semibold">속도 10 km/h → 페이스 변환</p>
            <p className="text-muted-foreground mt-1">10 km/h = 60 ÷ 10 = 6:00 min/km</p>
          </div>
          <div className="p-3 bg-muted rounded-lg">
            <p className="font-semibold">페이스 5:00 min/km → 속도 변환</p>
            <p className="text-muted-foreground mt-1">5:00 min/km = 60 ÷ 5 = 12.0 km/h</p>
          </div>
          <div className="p-3 bg-muted rounded-lg">
            <p className="font-semibold">마라톤 완주 예상 시간</p>
            <p className="text-muted-foreground mt-1">페이스 5:00/km × 42.195km = 3시간 30분 58초</p>
          </div>
        </div>
      ),
      faq: (
        <div className="space-y-4">
          {[
            { q: '페이스와 속도의 차이는 무엇인가요?', a: '페이스(min/km)는 1km를 달리는 시간, 속도(km/h)는 1시간에 달리는 거리입니다. 서로 역수 관계이며, 60을 페이스로 나누면 속도가 됩니다.' },
            { q: '마라톤 완주 목표 시간을 어떻게 설정하나요?', a: '원하는 페이스 × 42.195km로 계산합니다. 예: 5:30/km 페이스면 약 3시간 52분, 6:00/km이면 약 4시간 13분입니다.' },
            { q: '초보자에게 적절한 러닝 페이스는?', a: '대화가 가능한 페이스(7~9분/km)를 권장합니다. 처음에는 무리하지 말고 편한 속도로 시작하세요.' },
            { q: '오르막/내리막에서 페이스를 어떻게 조절하나요?', a: '오르막은 평지보다 10~20% 느리게, 내리막은 5~10% 빠르게 조절하십시오. 급격한 변화는 부상 위험이 있습니다.' },
            { q: '페이스를 향상시키려면 어떻게 훈련하나요?', a: '인터벌 트레이닝(빠르게-느리게 반복), 템포 런(고강도 지속), 장거리 러닝(지구력)을 병행하면 효과적입니다.' },
  ...
  ) : (
    {
      ...
      howToUse: (
        <ol className="space-y-3 text-sm list-decimal list-inside">
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">Select mode</span> → Choose speed-to-pace or pace-to-speed conversion</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">Enter values</span> → Input distance (km), time, or pace (min:sec/km)</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">Convert</span> → Click the Calculate button to perform the conversion</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">View results</span> → Check your calculated pace/speed and reference running paces</li>
        </ol>
      ),
      workedExamples: (
        <div className="space-y-3 text-sm">
          <div className="p-3 bg-muted rounded-lg">
            <p className="font-semibold">Speed 10 km/h → Pace conversion</p>
            <p className="text-muted-foreground mt-1">10 km/h = 60 ÷ 10 = 6:00 min/km</p>
          </div>
          <div className="p-3 bg-muted rounded-lg">
            <p className="font-semibold">Pace 5:00 min/km → Speed conversion</p>
            <p className="text-muted-foreground mt-1">5:00 min/km = 60 ÷ 5 = 12.0 km/h</p>
          </div>
          <div className="p-3 bg-muted rounded-lg">
            <p className="font-semibold">Marathon finish time prediction</p>
            <p className="text-muted-foreground mt-1">Pace 5:00/km × 42.195km = 3 hours 30 minutes 58 seconds</p>
          </div>
        </div>
      ),
      faq: (
        <div className="space-y-4">
          {[
            { q: 'What is the difference between pace and speed?', a: 'Pace (min/km) is the time to run 1km; speed (km/h) is the distance covered in 1 hour. They are reciprocals — dividing 60 by pace gives speed.' },
            { q: 'How do I set a marathon goal time?', a: 'Multiply your target pace by 42.195km. E.g., 5:30/km → ~3h52m, 6:00/km → ~4h13m.' },
            { q: 'What pace is appropriate for beginners?', a: 'A conversational pace (7–9 min/km) is recommended. Start at a comfortable speed without overexerting.' },
            { q: 'How should I adjust pace on hills?', a: 'Run 10–20% slower uphill and 5–10% faster downhill than your flat pace. Avoid sudden changes to prevent injury.' },
            { q: 'How can I improve my pace?', a: 'Combine interval training (fast-slow repeats), tempo runs (sustained high effort), and long runs (endurance) for effective improvement.' },
          ].map((f, i) => <FaqItem key={i} q={f.q} a={f.a} />)}
        </div>
      ),
    }
