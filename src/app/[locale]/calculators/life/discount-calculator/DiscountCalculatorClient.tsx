...
const DiscountCalculator = ({ infoSection }: DiscountCalculatorProps) => {
  ...
  const calculateDiscount = () => {
    if (mode === 'forward') {
      ...
      if (isNaN(original) || isNaN(discount) || original <= 0 || discount < 0 || discount > 100) {
        toast.error(t.alerts.invalidForward);
        return;
      }
      ...
      if (isNaN(original) || isNaN(sale) || original <= 0 || sale < 0) {
        toast.error(t.alerts.invalidReverse);
        return;
      }

      if (sale > original) {
        toast.error(t.alerts.saleTooHigh);
        return;
      }
