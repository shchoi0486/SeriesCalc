  ...
  const infoSection = isKo ? (
      ...
      <div>
        ...
        <ol className="space-y-4 text-sm text-muted-foreground">
          <li className="flex gap-3">
            ...
            <div><strong>이벤트 날짜 선택</strong> &rarr; 결혼기념일, 첫 만남 등 기준이 되는 이벤트 날짜를 선택합니다</div>
          </li>
          <li className="flex gap-3">
            ...
            <div><strong>이벤트 이름 입력</strong> &rarr; 탭에서 결혼, 연애, 기타 중 유형을 선택합니다</div>
          </li>
          <li className="flex gap-3">
            ...
            <div><strong>D-day 및 경과 시간 확인</strong> &rarr; 함께한 시간(년, 개월, 일)과 다음 기념일까지 남은 D-day를 확인합니다</div>
          </li>
        </ol>
      </div>
    `,
      ...
      <div>
        ...
        <div className="space-y-6 text-sm text-muted-foreground">
          <div className="p-3 bg-card rounded-md shadow-sm border border-border">
            <p className="font-semibold text-primary">예시: 결혼기념일 2020-06-15, 오늘 2026-08-25</p>
            <code className="block bg-muted p-2 rounded-md my-2 text-xs">
              총 경과일 = |2026-08-25 - 2020-06-15| = 2,262일
            </code>
            <code className="block bg-muted p-2 rounded-md my-2 text-xs">
              경과 기간 = 6년 2개월 10일
            </code>
            <p className="text-xs mt-2">함께한 시간: <strong>2,262일 (약 323주)</strong></p>
            <p className="text-xs">다음 기념일까지: <strong>D-day를 확인하세요</strong></p>
          </div>
        </div>
      </div>
    `,
    faq: (
      <div>
        ...
        <div className="space-y-5 text-sm text-muted-foreground">
          {[
            { q: '2월 29일(윤일)에 결혼하면 기념일은 언제인가요?', a: '윤년이 아닌 해에는 2월 28일 또는 3월 1일을 기념일로 사용하는 것이 일반적입니다. 법적 유효 기념일은 국가마다 다를 수 있습니다.' },
            { q: 'D-day가 음수이면 무엇을 의미하나요?', a: 'D-day가 음수이면 해당 기념일이 아직 오지 않았다는 뜻입니다. 예를 들어 D-30은 기념일이 30일 남았음을 의미합니다.' },
            { q: '시간대(타임존)는 어떻게 처리되나요?', a: '본 계산기는 날짜 단위로 계산하므로 시간대 차이는 고려하지 않습니다. 정확한 시·분 단위 계산이 필요한 경우 UTC 기준으로 입력하는 것이 좋습니다.' },
            { q: '반복 기념일도 지원하나요?', a: '매년 반복되는 결혼기념일, 생일 등은 기준일만 설정하면 다음 기념일을 자동으로 계산합니다. 매월 반복 등 별도 주기는 지원하지 않습니다.' },
            { q: '계산 결과를 내보낼 수 있나요?', a: '현재 결과를 이미지나 PDF로 저장하는 기능은 지원하지 않습니다. 필요 시 화면 캡처를 이용해 주세요.' },
    ...
    howToUse: `
      <div>
        ...
        <ol className="space-y-4 text-sm text-muted-foreground">
          <li className="flex gap-3">
            <span className="shrink-0 w-6 h-6 rounded-full bg-primary/10 text-primary text-xs font-bold flex items-center justify-center">1</span>
            <div><strong>Select event date</strong> &rarr; choose the date of your anniversary, wedding, first meeting, etc.</div>
          </li>
          <li className="flex gap-3">
            <span className="shrink-0 w-6 h-6 rounded-full bg-primary/10 text-primary text-xs font-bold flex items-center justify-center">2</span>
            <div><strong>Enter event type</strong> &rarr; select a tab for wedding, relationship, or other event</div>
          </li>
          <li className="flex gap-3">
            <span className="shrink-0 w-6 h-6 rounded-full bg-primary/10 text-primary text-xs font-bold flex items-center justify-center">3</span>
            <div><strong>View D-day and elapsed time</strong> &rarr; see years, months, days since the event and days remaining until the next anniversary</div>
          </li>
        </ol>
      </div>
    `,
    workedExamples: `
      <div>
        ...
        <div className="space-y-6 text-sm text-muted-foreground">
          <div className="p-3 bg-card rounded-md shadow-sm border border-border">
            <p className="font-semibold text-primary">Example: Wedding on 2020-06-15, today 2026-08-25</p>
            <code className="block bg-muted p-2 rounded-md my-2 text-xs">
              Total days = |2026-08-25 - 2020-06-15| = 2,262 days
            </code>
            <code className="block bg-muted p-2 rounded-md my-2 text-xs">
              Elapsed period = 6 years 2 months 10 days
            </code>
            <p className="text-xs mt-2">Time together: <strong>2,262 days (about 323 weeks)</strong></p>
            <p className="text-xs">Next anniversary: <strong>check D-day</strong></p>
          </div>
        </div>
      </div>
    `,
    faq: (
      <div>
        ...
        <div className="space-y-5 text-sm text-muted-foreground">
          {[
            { q: 'If we got married on February 29 (leap day), when is our anniversary?', a: 'In non-leap years it is common to celebrate on February 28 or March 1. The legally recognized anniversary may vary by country.' },
            { q: 'What does a negative D-day mean?', a: 'A negative D-day means the anniversary has not yet arrived. For example, D-30 means there are 30 days left until the anniversary.' },
            { q: 'How are time zones handled?', a: 'This calculator computes in day units, so time-zone differences are not considered. For precise hour/minute calculations, enter dates based on UTC.' },
            { q: 'Does it support recurring events?', a: 'Annual anniversaries such as weddings and birthdays are automatically calculated once you set the base date. Custom intervals like monthly recurrence are not supported.' },
            { q: 'Can I export the results?', a: 'Saving results as an image or PDF is not currently supported. Please use a screen capture if needed.' },
          ].map((f, i) => <FaqItem key={i} q={f.q} a={f.a} />)}
        </div>
      </div>
    )
