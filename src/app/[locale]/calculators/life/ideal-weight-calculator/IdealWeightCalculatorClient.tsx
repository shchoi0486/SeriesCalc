...
const IdealWeightCalculator = () => {
  ...
  const calculate = () => {
    ...
    if (isNaN(h) || h <= 0) {
      toast.error(isKo ? '키를 올바르게 입력해주세요.' : 'Please enter a valid height.');
      return;
    }
  ...
  const infoSection = isKo ? (
    {
      ...
      howToUse: (
        <ol className="space-y-3 text-sm list-decimal list-inside">
          ...
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">키 입력</span> → cm 단위로 키를 입력합니다</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">계산</span> → 계산 버튼을 클릭합니다</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">결과 확인</span> → 다양한 공식별 이상적 체중과 평균값을 확인합니다</li>
        </ol>
      ),
      workedExamples: (
        <div className="space-y-3 text-sm">
          <div className="p-3 bg-muted rounded-lg">
            <p className="font-semibold">남성 175cm</p>
            <p className="text-muted-foreground mt-1">BMI-22 기준 67.4kg, Devine 70.9kg, Robinson 72.7kg, Miller 68.6kg → 평균 약 69.9kg</p>
          </div>
          <div className="p-3 bg-muted rounded-lg">
            <p className="font-semibold">여성 160cm</p>
            <p className="text-muted-foreground mt-1">BMI-22 기준 56.3kg, Devine 53.7kg, Robinson 55.8kg, Miller 54.4kg → 평균 약 55.1kg</p>
          </div>
        </div>
      ),
      faq: (
        <div className="space-y-4">
          {[
            { q: '왜 여러 공식이 다른 결과를 보이나요?', a: '각 공식은 서로 다른 인구와 목적을 위해 개발되었습니다. Devine(1974)은 약물 투여량을 위해 만들어졌으며, BMI-22는 사망률 연구에 기반합니다.' },
            { q: '이상적 체중이 현재 체중과 크게 다르면?', a: '큰 차이가 반드시 건강 문제를 의미하지는 않습니다. 체중보다는 체지방률, 체력 수준, 대사 건강 지표에 집중하는 것이 중요합니다.' },
            { q: '체형(골격)에 따른 보정이 필요한가요?', a: '예. 어깨가 넓거나 골격이 큰 사람은 자연스럽게 체중이 더 나갈 수 있습니다. 골격 크기 보정을 고려해야 합니다.' },
            { q: '근육량이 많은 사람에게 적합한 공식은?', a: 'BMI-22가 가장 신뢰할 수 있는데, 이는 체지방이 아닌 사망률 데이터에 기반하기 때문입니다. 운동선수는 체중 목표보다 체지방률에 집중하는 것이 좋습니다.' },
            { q: '이상적 체중은 건강과 어떤 관련이 있나요?', a: 'BMI 18.5-24.9는 가장 낮은 사망률 위험과 관련이 있습니다. 이상적 체중의 10% 이내에 있는 것이 일반적으로 건강한 체중 범위입니다.' },
  ...
  ) : (
    {
      ...
      howToUse: (
        <ol className="space-y-3 text-sm list-decimal list-inside">
          ...
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">Enter height</span> → Enter your height in cm</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">Calculate</span> → Click the Calculate button</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">View results</span> → Check ideal weight by various formulas and their average</li>
        </ol>
      ),
      workedExamples: (
        <div className="space-y-3 text-sm">
          <div className="p-3 bg-muted rounded-lg">
            <p className="font-semibold">Male, 175cm</p>
            <p className="text-muted-foreground mt-1">BMI-22: 67.4kg, Devine: 70.9kg, Robinson: 72.7kg, Miller: 68.6kg → Average ~69.9kg</p>
          </div>
          <div className="p-3 bg-muted rounded-lg">
            <p className="font-semibold">Female, 160cm</p>
            <p className="text-muted-foreground mt-1">BMI-22: 56.3kg, Devine: 53.7kg, Robinson: 55.8kg, Miller: 54.4kg → Average ~55.1kg</p>
          </div>
        </div>
      ),
      faq: (
        <div className="space-y-4">
          {[
            { q: 'Why do different formulas give different results?', a: 'Each formula was developed for different populations and purposes. Devine (1974) was for drug dosing; BMI-22 is based on mortality research.' },
            { q: 'What if my ideal weight differs greatly from my current weight?', a: 'Large gaps don\'t necessarily indicate health problems. Focus on body composition, fitness level, and metabolic health markers instead.' },
            { q: 'Do I need to adjust for body frame size?', a: 'Yes. Broad-shouldered or big-boned individuals may naturally weigh more. Frame size adjustment accounts for this.' },
            { q: 'Which formula is best for very muscular people?', a: 'BMI-22 is most reliable as it\'s based on mortality data, not body composition. Athletes should focus on body fat percentage rather than weight targets.' },
            { q: 'How does ideal weight relate to health?', a: 'BMI 18.5-24.9 is associated with lowest mortality risk. Being within 10% of ideal weight generally indicates healthy weight range.' },
          ].map((f, i) => <FaqItem key={i} q={f.q} a={f.a} />)}
        </div>
      ),
    }
