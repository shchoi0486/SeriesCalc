...
const DueDateCalculator = () => {
  ...
  const calculate = () => {
    if (!lastPeriod) {
      toast.error(isKo ? '마지막 생리 첫날 날짜를 입력해주세요.' : 'Please enter the first day of your last period.');
      return;
    }

    const lmp = new Date(lastPeriod);
    if (isNaN(lmp.getTime())) {
      toast.error(isKo ? '올바른 날짜를 입력해주세요.' : 'Please enter a valid date.');
      return;
    }
  ...
  const infoSection = isKo ? (
    {
      ...
      howToUse: (
        <ol className="space-y-3 text-sm list-decimal list-inside">
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">마지막 생리 첫날 선택</span> → 마지막 월경 첫날 날짜를 입력합니다</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">계산 실행</span> → 계산 버튼을 클릭합니다</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">결과 확인</span> → 출산 예정일(EDC), 현재 주차, 삼분기 정보를 확인합니다</li>
        </ol>
      ),
      workedExamples: (
        <div className="space-y-3 text-sm">
          <div className="p-3 bg-muted rounded-lg">
            <p className="font-semibold">마지막 생리 첫날: 2025-01-01</p>
            <p className="text-muted-foreground mt-1">출산 예정일 = 2025-01-01 + 280일 = 2025년 10월 8일</p>
          </div>
          <div className="p-3 bg-muted rounded-lg">
            <p className="font-semibold">마지막 생리 첫날: 2025-06-15</p>
            <p className="text-muted-foreground mt-1">출산 예정일 = 2025-06-15 + 280일 = 2026년 3월 22일</p>
          </div>
        </div>
      ),
      faq: (
        <div className="space-y-4">
          {[
            { q: '출산 예정일의 정확도는 얼마나 되나요?', a: '출산 예정일은 ±2주(±14일) 정도 오차가 있을 수 있습니다. 예정일 당일에 출산하는 경우는 전체의 약 5%에 불과합니다.' },
            { q: '나겔 규칙(Naegele\'s Rule)의 한계는?', a: '불규칙한 월경 주기, 배란일 불확실, 다태 임신 등에서는 정확도가 떨어집니다. 초음파 측정이 더 정확할 수 있습니다.' },
            { q: '임신 주수는 어떻게 계산하나요?', a: '마지막 생리 첫날(LMP)로부터 경과된 일수를 7로 나눕니다. 의학적으로 임신 1주는 마지막 생리 첫날이 1주 0일이 됩니다.' },
            { q: '조산의 기준은 몇 주인가요?', a: '임신 37주 미만 출산을 조산(Preterm)으로 정의합니다. 32주 미만은 고위험 조산, 24~37주는 중기 조산에 해당합니다.' },
            { q: '만산(42주 이후)의 위험은 무엇인가요?', a: '42주 이후에는 태盘 기능 저하, 양수 감소, 거대아, 제대 사고 등 합병증 위험이 증가합니다. 일반적으로 41~42주 사이에 유도 분만을 고려합니다.' },
  ...
  ) : (
    {
      ...
      howToUse: (
        <ol className="space-y-3 text-sm list-decimal list-inside">
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">Select LMP date</span> → Enter the first day of your last menstrual period</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">Calculate</span> → Click the Calculate button</li>
          <li className="p-3 bg-muted rounded-lg"><span className="font-semibold">View results</span> → Check your estimated due date (EDC), current week, and trimester information</li>
        </ol>
      ),
      workedExamples: (
        <div className="space-y-3 text-sm">
          <div className="p-3 bg-muted rounded-lg">
            <p className="font-semibold">LMP: January 1, 2025</p>
            <p className="text-muted-foreground mt-1">Due Date = Jan 1, 2025 + 280 days = October 8, 2025</p>
          </div>
          <div className="p-3 bg-muted rounded-lg">
            <p className="font-semibold">LMP: June 15, 2025</p>
            <p className="text-muted-foreground mt-1">Due Date = Jun 15, 2025 + 280 days = March 22, 2026</p>
          </div>
        </div>
      ),
      faq: (
        <div className="space-y-4">
          {[
            { q: 'How accurate is the estimated due date?', a: 'The EDC has a margin of ±2 weeks (±14 days). Only about 5% of deliveries occur on the exact due date.' },
            { q: 'What are the limitations of Naegele\'s Rule?', a: 'Accuracy decreases with irregular cycles, uncertain ovulation, or multiple pregnancies. Ultrasound dating may be more accurate.' },
            { q: 'How is gestational age calculated?', a: 'Days elapsed since the first day of the last menstrual period (LMP) divided by 7. Medically, LMP is considered week 1, day 0.' },
            { q: 'What defines a preterm delivery?', a: 'Delivery before 37 weeks of gestation is classified as preterm. Before 32 weeks is very preterm; 24–37 weeks is moderate to late preterm.' },
            { q: 'What are the risks of post-term pregnancy (42+ weeks)?', a: 'Risks include declining placental function, low amniotic fluid, macrosomia, and cord complications. Induction is typically considered between 41–42 weeks.' },
          ].map((f, i) => <FaqItem key={i} q={f.q} a={f.a} />)}
        </div>
      ),
    }
