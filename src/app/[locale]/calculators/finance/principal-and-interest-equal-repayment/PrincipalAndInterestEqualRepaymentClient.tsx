    ...
    calculationFormula: (
      <div className="text-base leading-relaxed space-y-6">
        ...
        <div className="p-4 bg-muted rounded-lg border-l-4 border-primary">
          ...
          <BlockMath math={isKo ? "\\text{월 상환금} = \\dfrac{\\text{대출원금} \\times \\text{월이율} \\times (1 + \\text{월이율})^{\\text{상환개월수}}}{(1 + \\text{월이율})^{\\text{상환개월수}} - 1}" : "\\text{Monthly Payment} = \\dfrac{P \\times r \\times (1 + r)^{n}}{(1 + r)^{n} - 1}"} />
        ...
        <div className="p-4 bg-muted rounded-lg border-l-4 border-primary">
          ...
          <ul className="text-sm list-disc list-inside mt-2 space-y-2">
            <li>
              ...
              <BlockMath math={isKo ? "\\text{월 상환 이자} = \\text{전월 대출 잔액} \\times \\text{월이율}" : "\\text{Monthly Interest} = \\text{Previous Balance} \\times \\text{Monthly Rate}"} />
            </li>
            <li>
              <strong>{isKo ? '월 상환 원금 (점점 증가):' : 'Monthly principal (increasing):'}</strong>
              <BlockMath math={isKo ? "\\text{월 상환 원금} = \\text{월 상환금(고정)} - \\text{월 상환 이자}" : "\\text{Monthly Principal} = \\text{Fixed Payment} - \\text{Monthly Interest}"} />
            </li>
          </ul>
