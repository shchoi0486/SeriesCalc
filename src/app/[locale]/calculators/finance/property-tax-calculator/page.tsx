...
import CalculatorClient from "./PropertyTaxCalculatorClient";
...
export function generateMetadata({
  ...
  return buildCalculatorMetadata(params.locale, "/calculators/finance/property-tax-calculator", "finance", "property-tax-calculator");
}
      ...
      <>
        ...
        <div className="space-y-4 mt-4">
          <div className="p-4 pl-3 bg-muted rounded-lg border-l-4 border-primary">
            ...
            <BlockMath math={isKo ? "\\text{공정가액} = \\text{공시가격} \\times \\text{공정가액비율}" : "\\text{Fair Value} = \\text{Official Price} \\times \\text{Fair-Value Ratio}"} />
            <BlockMath math={isKo ? "\\text{과세표준} = \\text{공정가액} - \\text{기본공제}" : "\\text{Tax Base} = \\text{Fair Value} - \\text{Basic Deduction}"} />
